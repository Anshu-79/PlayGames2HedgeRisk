...
def violation_ratio(y_true: np.ndarray, var_pred: np.ndarray,
...
def kupiec_test(y_true: np.ndarray, var_pred: np.ndarray,
...
def christoffersen_test(y_true: np.ndarray, var_pred: np.ndarray) -> dict:
    ...
    L_ind = (
        safe_log(1 - pi_01) * n00 + safe_log(pi_01) * n01 +
        safe_log(1 - pi_11) * n10 + safe_log(pi_11) * n11
    )
    L_iid = (
        safe_log(1 - pi) * (n00 + n10) +
        safe_log(pi) * (n01 + n11)
    )
...
def evaluate_var(y_true: np.ndarray, var_pred: np.ndarray,
                 quantile: float = 0.95) -> dict:
    ...
    return {
        ...
        **{f"christoffersen_{k}": v for k, v in christoffersen_test(y_true, var_pred).items()},
    }
...
def tail_mean_error(y_true: np.ndarray, cvar_pred: np.ndarray,
                    var_pred: np.ndarray) -> float:
...
def joint_var_cvar_score(y_true: np.ndarray, var_pred: np.ndarray,
                         cvar_pred: np.ndarray, quantile: float = 0.95) -> float:
    """
    ...
    q_loss = np.mean(np.where(err_var < 0, q * err_var, (q - 1) * err_var))
...
def es_backtest(y_true: np.ndarray, var_pred: np.ndarray,
                cvar_pred: np.ndarray) -> dict:
    """
...
def evaluate_cvar(y_true: np.ndarray, var_pred: np.ndarray,
                  cvar_pred: np.ndarray, quantile: float = 0.95) -> dict:

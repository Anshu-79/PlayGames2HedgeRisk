...
from src.utils.model_factory import MODEL_REGISTRY, build_model
...
@click.option("--model", "-m", default=None, help="Override model type from config")
def rolling(config, model):
    ...
    logger.info(f"Model: {model_obj}")
    run_rolling(model_obj, returns, features, cfg)


@cli.command()
@click.option("--config", "-c", required=True, help="Path to experiment YAML config")
@click.option("--model", "-m", default=None, help="Override model type from config")
def ood(config, model):
...
def benchmark(config, model, all_models):
    ...
    if model:
        ...
        models_to_run = [cfg["model"]["type"]]

    summary = {}

    for model_type in models_to_run:
        cfg_local = copy.deepcopy(cfg)
        cfg_local["model"]["type"] = model_type

        logger.info(f"Benchmarking: {model_type}")

        try:
            model_obj = build_model(cfg_local)
            result = run_rolling(model_obj, returns, features, cfg_local)
